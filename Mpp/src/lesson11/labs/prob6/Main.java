...
public class Main {
    ...
    public static <T> boolean contains(List<? extends T> list, T target, BiPredicate<T,? super T> pred2) {
        //could return list.contains(s), but this does not generalize
        for (T t : list) {
            if (t == null && target == null) return true;
            if (target == null || t == null) return false;
            if (pred2.test(t, target)) return true;
        }
        return false;
    ...
    public static void test2() {
        ...
        boolean foundIt = Main.contains(list, e,
